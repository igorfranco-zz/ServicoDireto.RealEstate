...
namespace SpongeSolutions.ServicoDireto.Admin.Controllers
{
    ...
    {
        [HttpGet]
        public IHttpActionResult ListActive(string idCountry, int idStateProvince)
        {
            return Ok(ServiceContext.CityService.GetAllActive(idCountry, idStateProvince));
        }
        ...
        {
            page--;
            var result = ServiceContext.CityService.GetAll(out recordCount, startRowIndex: maximumRows * page, maximumRows: maximumRows);
            return Ok(new
            {
        ...
        [HttpGet]
        public IHttpActionResult GetByID(int idCity)
        {
            return Ok(ServiceContext.CityService.Get(idCity));
        }

        [HttpPost]
        public IHttpActionResult Save([FromBody]City record)
        {
            ...
            {
                if (record.IDCity.HasValue)
                {
                    record.ModifyDate = DateTime.Now;
                    record.ModifiedBy = SiteContext.ActiveUserName;
                    Services.ServiceContext.CityService.Update(record);
                ...
                else
                {
                    ...
                    record.ModifyDate = DateTime.Now;
                    record.ModifiedBy = SiteContext.ActiveUserName;
                    Services.ServiceContext.CityService.Insert(record);
        ...
        public IHttpActionResult Deactivate([FromBody]int[] id)
        {
            if (id != null && id.Count() > 0)
                ServiceContext.CityService.Inactivate(id);
