...
CREATE procedure [dbo].[pElement_ListBookmarked]
(
	@IDCustomer int,
	@IDCulture varchar(5)
)
...
begin
	select 
		...
		from Element e with(nolock)
			...
			inner join elementbookmarked eb on (e.idelement = eb.idelement)
		where eb.idcustomer = @IDCustomer
		and ec.idculture = @idCulture	
end
